...
pub fn main() !void {
    ...
    // PART 1
    {
        ...
        outer: while (lines.next()) |line| {
            var last_number: ?Int = null;
            var second_last_number: ?Int = null;
            ...
            while (i < line.len) : (i += 1) {
                ...
                const number = std.fmt.parseInt(Int, line[start..i], 10) catch unreachable;
                defer last_number = number;

                if (last_number == null) continue;

                const curr_increasing = number > last_number.?;
                const diff = if (curr_increasing) number - last_number.? else last_number.? - number;

                if ((second_last_number != null and (last_number.? > second_last_number.?) != curr_increasing) or diff < 1 or diff > 3) {
                    continue :outer;
                }
...
fn nextInt(T: type, slice: *[]const u8) ?T {
    const start = 0;
    var i = 0;
    return while (i < slice.len) switch (slice[i]) {
        '0'...'9' => i += 1,
        else => {
            slice.* = slice.*[i..];
            break std.fmt.parseInt(T, slice[start..i]) catch unreachable;
        },
    };
}
