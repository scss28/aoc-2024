...
pub fn main() void {
    ...
    // PART 1
    {
        ...
        state: switch (State.start) {
            .start => switch (data[i]) {
                0 => {},
                else => if (data.len - i >= 8 and std.mem.eql(u8, data[i .. i + 4], "mul(")) {
                    i += 4;
                    lhs_i = i;
            ...
            .first_num, .second_num => |state| {
                const byte = data[i];
                i += 1;
                switch (byte) {
                    '0'...'9' => continue :state state,
                    ',' => if (state == .first_num) {
                        rhs_i = i;
                        continue :state .second_num;
                    },
                    ')' => if (state == .second_num) {
                        const lhs = std.fmt.parseInt(
                            Int,
                            data[lhs_i .. rhs_i - 1],
                            10,
                        ) catch unreachable;

                        const rhs = std.fmt.parseInt(
                            Int,
                            data[rhs_i .. i - 1],
                            10,
                        ) catch unreachable;
        ...
        state: switch (State.start) {
            .start => switch (data[i]) {
                0 => {},
                    ...
                    const left = data.len - i;
                    if (do and left >= 8 and std.mem.eql(u8, data[i .. i + 4], "mul(")) {
                        i += 4;
                        lhs_i = i;
                        continue :state .first_num;
                    } else if (left >= 7 and std.mem.eql(u8, data[i .. i + 7], "don't()")) {
                        i += 7;
                        do = false;
                    } else if (left >= 4 and std.mem.eql(u8, data[i .. i + 4], "do()")) {
                        i += 4;
                        do = true;
                    } else i += 1;
